...
- ⚡ **Lightning Fast App Launcher**: Instant fuzzy search with auto-launch capability.
...
- 🔗 **Native Deep Link Support**: Execute custom URI schemes (e.g. `lazylogs://add?text=makan%20siang%2025k`) or use the built-in `log <text>` financial tracking shortcut.
...
- 🖥️ **Experimental Retro TUI Mode**: 100% pure ASCII terminal UI with framed window boxes (`tui`).
...
- 🛡️ **Minimal Permissions**: Uses `QUERY_ALL_PACKAGES` for app discovery and `SET_ALARM` for timers.
